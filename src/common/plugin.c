...
plugin_auto_load (session *sess)
{
	...
	for_files ("./plugins", "pcdns.dll", plugin_auto_load_cb);
	for_files ("./plugins", "pcexec.dll", plugin_auto_load_cb);
	for_files ("./plugins", "pclua.dll", plugin_auto_load_cb);
	for_files ("./plugins", "pcperl.dll", plugin_auto_load_cb);
	for_files ("./plugins", "pcpython.dll", plugin_auto_load_cb);
	for_files ("./plugins", "cap_sasl.dll", plugin_auto_load_cb);
	for_files ("./plugins", "pcwinamp.dll", plugin_auto_load_cb);
	for_files ("./plugins", "pcwmpa.dll", plugin_auto_load_cb);
	for_files ("./plugins", "xtray.dll", plugin_auto_load_cb);
...
xchat_pluginpref_set_str_real (xchat_plugin *pl, const char *var, const char *value, int mode) /* mode: 0 = delete, 1 = save */
{
	...
	sprintf (confname, "plugins_%s.conf", canon);
	g_free (canon);
...
xchat_pluginpref_list (xchat_plugin *pl, char* dest)
{
	...
	sprintf (confname, "plugins_%s.conf", token);
	g_free (token);
