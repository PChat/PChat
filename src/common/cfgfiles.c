...
get_xdir_fs (void)
{
	if (!xdir_fs)
	{
			...
			{
				xdir_fs = g_strdup_printf ("%s\\" "PChat", out);
			}
	}
...
{
	if (!xdir_fs)
		xdir_fs = g_strdup_printf ("%s/.config/" XCHAT_DIR, g_get_home_dir ());
...
default_file (void)
{
	...
	if (!dfile)
	{
		...
		sprintf (dfile, "%s/pchat.conf", get_xdir_fs ());
	}
	return dfile;
}
