...
#ifdef USE_PLUGIN
	if (!arg_skip_plugins)
		plugin_auto_load (sess);	/* autoload ~/.pchat *.so */
#endif
...
enum_windows_impl (HWND current_window, LPARAM lParam)
{
	...
	if (strcmp (window_name, "PChat") == 0)
	{
		...
		if (strstr (module_path, "pchat.exe"))	/* We've found it, stop */
		{
...
main (int argc, char *argv[])
{
	...
	if (prefs.gui_one_instance && !portable_mode ())
	{
		...
		mutex = CreateMutex (NULL, TRUE, "Local\\pchat");
		error = GetLastError ();
